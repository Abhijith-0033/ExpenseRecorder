...
// Dynamic category list — loaded from DB on each validation call so new custom categories work instantly
...
function validateExpenseBody(body, isPartial = false) {
  const errors = [];
  ...
  if (!isPartial || body.category !== undefined) {
    if (!body.category) {
      ...
      errors.push(`Invalid category. Please select a valid category from the list.`);
    }
  }
...
router.post('/expenses', (req, res) => {
  const errors = validateExpenseBody(req.body, false);
  if (errors.length > 0) {
...
router.put('/expenses/:id', (req, res) => {
  ...
  try {
    ...
    ).run(title, amount, category, date, note, id);
...
router.get('/summary', (req, res) => {
  try {
    ...
    const breakdown = allCategoryNames.map(cat => ({
      category: cat,
      total:    map[cat] ? Number(map[cat].total.toFixed(2)) : 0,
      count:    map[cat] ? map[cat].count : 0
    }));

    const totalSpent = Number(breakdown.reduce((sum, c) => sum + c.total, 0).toFixed(2));
